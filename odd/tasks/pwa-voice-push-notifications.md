...
### T4 — done (2026-09-21) — last task, feature complete pending manual verification

- `apps/web/vite.config.ts`: `VitePWA` switched to `strategies: 'injectManifest'` + `srcDir: 'src'` + `filename: 'sw.ts'`; removed the old `workbox: {...}` options block (behavior moved into the hand-written SW below).
- `apps/web/src/sw.ts` (new): `precacheAndRoute(self.__WB_MANIFEST)`, `skipWaiting()`/`clientsClaim()`, a `NetworkFirst` route for `/api/*` reproducing the old runtimeCaching entry, `push` listener (parses JSON payload, `showNotification`), `notificationclick` listener (focuses an existing tab or opens a new one at the notification's `url`). File-local `declare const self: ServiceWorkerGlobalScope` was enough to typecheck cleanly alongside the rest of the DOM-typed `src/` — no separate tsconfig needed.
- `apps/web/src/lib/use-push-subscription.ts` (new): `usePushSubscription()` — support check (`serviceWorker`/`PushManager`/`Notification`), subscription-status effect, `subscribe`/`unsubscribe` mutations calling `POST`/`DELETE /push/subscribe`, canonical `urlBase64ToUint8Array` VAPID-key helper.
- `apps/web/src/app/_app/settings.tsx`: new "Notificaciones" `Section`/`Toggle`, reusing the existing components unmodified, slotted between "Privacidad" and the logout divider; the whole section is hidden (not disabled) when push isn't supported.
- Added `workbox-core`/`workbox-precaching`/`workbox-routing`/`workbox-strategies` as direct deps of `apps/web` (were only transitive via `workbox-build`, not resolvable at the app level) — via `corepack pnpm add`, run inside `apps/web`.
- **Important finding, not fixed by this task**: `vite.config.ts`'s `loadEnv(mode, process.cwd(), '')` resolves `process.cwd()` to `apps/web`, not the repo root, when run through `pnpm --filter` — confirmed empirically. So the root `.env` is **not** read by `pnpm dev:web`/`pnpm build` today. This was invisible until now because `VITE_API_URL` has a `?? 'http://localhost:3001'` fallback; `VITE_VAPID_PUBLIC_KEY` has none, so `subscribe()` throws `'VITE_VAPID_PUBLIC_KEY no está configurada'` until an `apps/web/.env` exists (already covered by the root `.gitignore`'s blanket `.env`/`.env.*` patterns). Could not create it — same Bash/Write permission-denied wall as the root `.env` earlier. Asked the user to add `apps/web/.env` with the same public key.
- Verified independently (re-ran, didn't just trust the sub-agent's report): `apps/web` typecheck clean (`tsc -p tsconfig.app.json --noEmit`), production `vite build` succeeds — plugin summary confirms `mode injectManifest` (not `generateSW`), `dist/sw.js` built and greppable for `notificationclick`/`precacheAndRoute`. **Not verified**: the actual subscribe/permission-prompt/push-delivery/notification-click flow in a live browser — no browser automation tool available this session. Both this and T5's voice input need a manual pass by the user before either is trusted as working end to end, not just "builds clean."
